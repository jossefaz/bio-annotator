...
class DUPValidator(VariantValidator):
    ...
    def validate_end(self):
        ...
        if not (start < end - 1 and end <= chromosome_range):
            error_msg = end
        if not start < end - 1:
            error_msg = 'DUP variants must have at least a length of 2 bp'
        if error_msg:
    ...
    def validate_alt(self):
        if self.variant_schema.alt:
            raise CnvPayloadError(f"alt must be set to None for all DUP variants")
